...
def Yolov2Loss(y_true, y_pred):
    ...
    predict_bxby = cell_offset_table() + (tf.sigmoid(predict_txty) * 32)  # ? * 13 * 13 * 5 * 2
    ...
    xy_loss = 5 * anchor_mask * responsible_mask * K.square(label_bxby / 32 - predict_bxby / 32)
    box_loss = xy_loss

    # tf.print("\n- xy loss:", tf.reduce_sum(xy_loss), output_stream=sys.stdout)

    # Loss 함수 2번
    wh_loss = 5 * anchor_mask * responsible_mask * K.square(K.sqrt(label_bwbh / 32) - K.sqrt(predict_bwbh / 32))
    box_loss += wh_loss
